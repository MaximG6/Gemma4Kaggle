"""
VoiceBridge QLoRA Fine-Tune V2
================================
Re-trains on the V2 dataset (current triage_system.txt prompt) with LoRA
applied to LM text layers ONLY — audio/vision tower excluded.

Changes from finetune.py (V1):
  - dataset  : data/finetune_train_v2.jsonl
  - adapter  : models/voicebridge-gemma4-triage-adapter-v2/
  - merged   : models/voicebridge-merged-v2/
  - log      : runs/finetune_log_v2.jsonl
  - config   : runs/finetune_config_v2.json
  - LoRA     : filtered to model.layers.* only (no vision/audio tower modules)
...
Usage (from voicebridge/ repo root, conda env voicebridge active):
    ...
    python scripts/finetune_v2.py --dry-run
    python scripts/finetune_v2.py --merge-only
"""
...
DEFAULTS = dict(
    ...
    dataset_path      = str(_REPO_ROOT / "data" / "finetune_train_v2.jsonl"),
    adapter_output    = str(_REPO_ROOT / "models" / "voicebridge-gemma4-triage-adapter-v2"),
    merged_output     = str(_REPO_ROOT / "models" / "voicebridge-merged-v2"),
    log_path          = str(_REPO_ROOT / "runs" / "finetune_log_v2.jsonl"),
    # LoRA
    lora_rank         = 32,
    lora_alpha        = 64,
    lora_dropout      = 0.075,
...
# LoRA param name substrings that indicate audio/vision tower components.
# Used to freeze those params after get_peft_model attaches LoRA everywhere.
_EXCLUDED_PREFIXES = (
    "vision", "audio", "image", "multi_modal", "multimodal",
    "siglip", "clip", "whisper",
)
...
def _freeze_audio_tower_lora(model) -> int:
    """
    After get_peft_model attaches LoRA to every matching suffix, freeze the
    LoRA weights that belong to vision/audio tower components by setting
    requires_grad=False. Returns the count of params frozen.

    This is more reliable than passing full module paths to target_modules,
    which Unsloth may reject (it validates target_modules itself before PEFT).
    """
    frozen = []
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        low = name.lower()
        if any(x in low for x in _EXCLUDED_PREFIXES):
            param.requires_grad_(False)
            frozen.append(name)

    if frozen:
        print(f"  [audio/vision excluded] froze {len(frozen)} LoRA params", flush=True)
        for n in frozen[:3]:
            print(f"    {n}", flush=True)
        if len(frozen) > 3:
            print(f"    … and {len(frozen) - 3} more", flush=True)
    else:
        print("  [note] No audio/vision LoRA params found — model may be text-only", flush=True)
    return len(frozen)
...
def run_finetune(cfg: dict) -> None:
    ...
    try:
        ...
        print("[ERROR] trl is not installed.\n        pip install trl", file=sys.stderr)
        sys.exit(1)
    ...
    _tick(f"      Freezing audio/vision tower LoRA params …")
    n_frozen = _freeze_audio_tower_lora(model)

    total_params     = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    _tick(f"      Trainable params: {trainable_params:,}  "
          f"({100 * trainable_params / total_params:.2f}% of {total_params:,})\n"
          f"      V1 had 84,803,584 trainable params — V2 should be less ({n_frozen} frozen)")
    ...
    finetune_config = {
        ...
        "lora_dropout":          cfg["lora_dropout"],
        "target_modules":        cfg["target_modules"],
        "audio_tower_lora_frozen": n_frozen,
        "audio_tower_excluded":  True,
        "load_in_4bit":          cfg["load_in_4bit"],
        "epochs":                cfg["epochs"],
    ...
    config_json_path = runs_dir / "finetune_config_v2.json"
    ...
    _tick("\n" + "=" * 64 + "\n"
          "VoiceBridge V2 Training Summary\n"
          + "=" * 64 + "\n"
          ...
          f"({100 * trainable_params / total_params:.2f}% of {total_params:,})\n"
          f"  Audio/vision excluded    : yes\n"
          f"  Adapter saved            : {cfg['adapter_output']}"
          + (f"\n  Merged model saved       : {cfg['merged_output']}"
             if not cfg.get("dry_run") else "") + "\n"
          f"  Loss log                 : {cfg['log_path']}\n"
          f"  Config                   : {runs_dir / 'finetune_config_v2.json'}\n"
          + "=" * 64)


def parse_args() -> dict:
    parser = argparse.ArgumentParser(
        description="VoiceBridge V2 QLoRA fine-tune — Gemma 4 E4B, LM layers only"
    )
...
def run_merge_only(cfg: dict) -> None:
    ...
    _tick("Loading V2 base model + adapter …")
    model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    _tick(f"\nSaving merged model : {merged_path}")
    model.save_pretrained_merged(
        merged_path,
        tokenizer,
        save_method = "merged_16bit",
    )
    _tick(f"  Merged model saved : {merged_path}")
...
if __name__ == "__main__":
    ...
    print("=" * 64)
    print("VoiceBridge QLoRA Fine-Tune V2  (LM layers only — audio/vision excluded)")
    print("=" * 64)
    print(f"  Model          : {cfg['model_id']}")
    print(f"  Dataset        : {cfg['dataset_path']}")
    print(f"  LoRA rank/alpha: {cfg['lora_rank']} / {cfg['lora_alpha']}")
    print(f"  Target modules : {cfg['target_modules']}  (will be filtered to LM layers)")
